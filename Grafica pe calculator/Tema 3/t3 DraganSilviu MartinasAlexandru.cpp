	...
	public:
		void grila(int nr_linii, int nr_coloane)
		{
			...
			glBegin(GL_LINES);
				...
				glVertex2f(-0.99 + (nr_coloane * latura_scalata2),0.99-(8*latura_scalata1));
				glVertex2f(-0.99,0.99);
				glVertex2f(-0.99 + (nr_coloane * latura_scalata2),0.99-(5*latura_scalata1));
			glEnd();
			...
			AfisareSegmentDreapta3Alex(0,-nr_linii,-nr_coloane,-8,latura_scalata1,latura_scalata2);
			AfisareSegmentDreapta3Alex(-nr_coloane,-5,0,0,latura_scalata1,latura_scalata2);
			AfisareSegmentDreapta3Alex(-nr_coloane,-4,-1,0,latura_scalata1,latura_scalata2);
			AfisareSegmentDreapta3Alex(-nr_coloane,-6,0,-1,latura_scalata1,latura_scalata2);
				
		}

		double ecuatieDreapta(double x, double y, int x0, int y0, int xn, int yn) {
			double dy = yn - y0;
			double dx = xn - x0;
			double a = dy;
			double b = -dx;
			double c = dx * y0 - dy * x0;
						
			return (double)(a*x + b*y + c);
		}

		void AfisareSegmentDreapta3Alex(int x0, int y0, int xn, int yn, double latura_scalata1, double latura_scalata2){
			double xm,ym,d;
			int x = x0,y = y0;
			writePixel(x,y,latura_scalata1,latura_scalata2);
			if (x > xn)
				xn = -xn;
			while (x<xn){
				xm = x+1;
				ym = y + 0.5;
				d = ecuatieDreapta(xm,ym,x0,y0,xn,yn);
				printf("%f\n", d);
				if(d<=0) {
					x++;
				}
		...
		void AfisareSegmentDreapta3AlexGros(int x0, int y0, int xn, int yn, double latura_scalata1, double latura_scalata2){
			double xm,ym,d;
			int x = x0,y = y0;
			writePixel(x,y,latura_scalata1,latura_scalata2);
			if (x > xn)
			...
			while (x<xn){
				...
				writePixel(x,y,latura_scalata1,latura_scalata2);
				writePixel(x-1,y,latura_scalata1,latura_scalata2);
				writePixel(x+1,y,latura_scalata1,latura_scalata2);			
			}
		}
		...
		void writePixel(int index_linie, int index_coloana, double latura_scalata1, double latura_scalata2)
		{
			...
			glBegin(GL_POLYGON);
			double p1,p2;
			index_coloana = abs(index_coloana);
			index_linie = abs(index_linie);
